...
groups = []
with open("grouping_log.txt", "w", encoding="utf-8") as f:
    for line in raw_lines:
        ...
        for group in groups:
            ...
            if same_row or close_vertically:
                if is_new_prod and has_prod and horiz_dist > 20:
                    f.write(f"Skipping merge: {line['text']} vs Group with horiz_dist={horiz_dist}\n")
                    continue
                if vertical_dist > 35:
                    continue
        ...
        if best_group is not None:
            ...
            f.write(f"Merged {line['text']} into group. Score={best_score}\n")
        else:
            groups.append([line])
            f.write(f"New group: {line['text']}\n")
